...
\section{Theory for Basic Flow Matching}
...
We first derive the expression of $u_t$. On the one hand, according to the continuity function:
\begin{equation}
  \label{continuity}
\frac{d}{d t} p_t(x)+\operatorname{div}\left(p_t(x) u_t(x)\right)=0
\end{equation}
...
\begin{equation}
  \label{conditioned}
  \begin{aligned}
  \frac{d}{d t} p_t(x)&=\int\left(\frac{d}{d t} p_t\left(x|x_1\right)\right) q\left(x_1\right) d x_1\\
  &=-\int\operatorname{div}\left(u_t\left(x|x_1) p_t(x|x_1)\right)q(x_1) d x_1\right.\\
  &=-\operatorname{div}\left(\int\left(u_t\left(x|x_1) p_t(x|x_1)\right)q(x_1) d x_1\right.\right)\\
  \end{aligned}
\end{equation}

By comparing Eqs. (\ref{continuity}) and (\ref{conditioned}), we have:
\begin{equation}
  u_t=\frac{\int u_t\left(x|x_1\right) p_t\left(x|x_1\right) q\left(x_1\right) d x_1}{p_t(x)}.
\end{equation}

The middle term of Eq. (\ref{L_FMBase}) can be transformed using the expection notation:
